...
public class SessionService {
    ...
    public ResponseDto<DashboardResponseDto> getDashboardData(String counselorEmail, Long clientId) {
        List<String> defaultEmotions = Arrays.asList("행복", "중립", "불안", "당황", "슬픔", "분노", "혐호");
        // 총 내담자 수
        ...
        if (clientId == 0) {
                    ...
                    session -> {
                        ...
                        List<AnalysisDto> analysisDtoList = emotionAnalysisReports.stream().map(
                                emotionAnalysisReport -> {
                                    ...
                                    return new AnalysisDto(emotionAnalysisReport.getId(), emotionAnalysisReport.getSpeakerLabel(), emotionAnalysisReport.getSentenceText(), emotionAnalysisReport.getDominantEmotion(), convertStringToList(emotionAnalysisReport.getKeywords()));
                                }
                        ).toList();
                        return new CounselingDetailDto(session.getId(), session.getClient().getName(), convertTimestampToString(session.getSessionDate()), session.getMinuteOfCounseling(), analysisDtoList);
                    }
            ).toList();
            ...
            DashboardResponseDto response = new DashboardResponseDto(totalClientCount, totalSessionCount, convertMinutesToTimeString(totalCounselingTime), mostFrequentEmotion, emotionCounts, counselingDetails);
